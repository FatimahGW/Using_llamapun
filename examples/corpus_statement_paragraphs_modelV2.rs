...
// Record the Time For Measuring the Performance 
 ...
 // Read input arguments
  ...
  let corpus_path = match input_args.next() { //corpus_path : the html files names
   ...
   let paragraph_model_file = match input_args.next() { // paragraph_model_file the name of the output tar file
    Some(path) => path,
    None => "statement_paragraphs.tar".to_string(),
  };
  let discard_math = match input_args.next() { //Change her for including/dicluding the math formula
  ...
  // names : should be the HTML file's name match the varible...corpus_path
  ...
  let tar_builder = Arc::new(Mutex::new(TarBuilder {
    ...
    names: HashSet::new(), //is it possible to Use the HTML file's name here instead of the hash
  }));
  //Reading files Starts here <.....
   let mut corpus = Corpus::new(corpus_path);// corpus: HTML file each time
...
//This part could be removed
  ...
  let catalog = corpus.catalog_with_parallel_walk(|doc| {
    extract_document_statements(doc, tar_builder.clone(), discard_math) //<..... Calling the main function
  });
// the following println Could be removed but still useful
 /*
 println!(
 ...
 let mut builder_lock = tar_builder.lock().unwrap();
  println!(
    "{:?} paragraphs written to .tar destination (discarded duplicate SHA256-based filenames)",//print the excat number of paragraphes (e.g 124 paragraphs written to .tar destination)
    builder_lock.count
  );
  builder_lock
    .builder
...
fn extract_document_statements(
  ...
  discard_math: bool,
) -> HashMap<String, u64> { //Returen HashMap which Should be removed
